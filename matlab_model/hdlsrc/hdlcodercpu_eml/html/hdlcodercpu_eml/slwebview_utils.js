...
SvgLoader.prototype.execute = function() 
{
    if (hasAdobeSVGPlugin()) {
        ...
        document.write('<iframe tabindex="-1" height="100%" width="100%" '
                       + 'style="border: 0" src="' + this.filename + '"></iframe>');
    } else {
...
{
    var isInUnixWebkit = (navigator.appVersion.indexOf('Qt/4.7.1') !== -1 && navigator.platform.indexOf('Win') === -1);
    var isIceBrowser = (navigator.appVersion.indexOf('ICEbrowser/v6_0_2') !== -1);
    if (isInUnixWebkit || isIceBrowser)
        return false;
    else
        return true;
}
